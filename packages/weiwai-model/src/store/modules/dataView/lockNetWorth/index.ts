...
import { lockNetWorthState } from '@services/types/dataShow/lockNetWorth'
...
export const lockNetWorthStore = defineStore('lockNetWorth', {
  state: (): lockNetWorthState => ({
    treeList: [],
    hasParent: false,
    checkedKeys: [],
    expandedKeys: [],
    ...
    tableColumn: [
      ...
      { prop: 'taskDate', label: '估值日期', align: 'center', width: '96' },
      { prop: 'bizDate', label: '任务日期', align: 'center', width: '96' },
  ...
  actions: {
    ...
    async inquire(val?: any) {
      ...
      this.formObject = cloneDeep(val)
      this.loadingShow = true
      const res = await vlaData.netDataLockGetListPost({
        ...val,
        ...{
          startPage: this.pageInfo.startPage,
          pageSize: this.pageInfo.pageSize,
          customerCode: this.checkedTreeNode.map((i: any) => i.id)
        }
      })
